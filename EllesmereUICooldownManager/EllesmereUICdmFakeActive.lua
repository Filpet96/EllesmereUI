...
EvalCdStateNow = function()
    ...
    for r = 1, #_cdStateRules do
        ...
        if eff or soundKey then
            ...
            if srcKey < 0 and GetInventoryItemID then -- re-resolve trinket slot; baked slot can be stale at login/swap
                local itemID = -srcKey
...
ns.FullCDMRebuild = function(reason)
    if ns._cdmBumpSoundSettle then ns._cdmBumpSoundSettle() end -- drop false CD-ready arms during rebuild
